...
export default function Privacyicons({ note, callBack, setView, setTagNotes }) {
  ...
  const deleteNote = async () => {
    ...
    try {
      const response = await fetch(
        `${config.apiUrl}/notes/deleteNote/${note._id}/${user._id}`
      );
  ...
  const removeTag = async () => {
    ...
    try {
      const response = await fetch(
        `${config.apiUrl}/user/removeTag/${user.username}/${note._id}/${note.userId}`,
        {
          headers: {
  ...
  const changePrivacy = async (e, label) => {
    ...
    try {
      const response = await fetch(
        `${config.apiUrl}/notes/updateNote/${note._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
